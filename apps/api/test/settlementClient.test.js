...
test('default client is disabled when env is unset; every call is a null no-op', async () => {
  ...
  assert.equal(await settlementClient.credit({ userId: 'u_1', chain: 'lisk', asset: 'USDC', amount: '100', idempotencyKey: 'k' }), null);
  assert.equal(await settlementClient.transfer({ fromUserId: 'u_1', toUserId: 'u_2', asset: 'USDC', amount: '1', idempotencyKey: 'k' }), null);
  assert.equal(await settlementClient.quote({ chain: 'lisk', asset: 'USDC', net: '100' }), null);
...
test('passes the idempotency key through on mutations', async () => {
  ...
  const result = await client.credit({ userId: 'u_1', chain: 'lisk', asset: 'USDC', amount: '100', idempotencyKey: 'api:tx_9' });
